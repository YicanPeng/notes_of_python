...
# 聚合函数,.apply由于是直接重载np函数,因此axis表示运算方向;
# .drop和与series计算是pandas函数,因此axis表示传播方向
""" axis表示只负责行进不参与运算的维度,而参数的索引用于筛选series;
   以2维pd为例:axis=0,则成列执行函数,并且参数在axis=0的轴上对齐
   .drop删除整行;与series计算,series的元素与pd行对齐元素执行运算 """
...
""" fillna接受series参数 """
# sr102_19=df102_19.mean(axis=0)
# p(sr102_19)
""" 现阶段fillna只能逐列填充,只能axis=0 """
